...
// The issue here is that this is not guaranteed to work. In practical terms, this may fail if there is a gc run between the two conversions that relocates the backing array (and modifies any pointers to it).
...
// if the pattern is not present, usual aguing about the language fails. The only thing I see that would work is
...
// """
//
// Essentially, this is needed to (atomically) convert a PAIR of pointers to a PAIR of uintptr.
